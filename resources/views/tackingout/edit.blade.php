...
@section('title') Tambah Tackingout @endsection

@section('header')
 <div class="header-body">
    <div class="row align-items-center py-4">
      <div class="col-lg-6 col-7">
        <h6 class="h2 text-white d-inline-block mb-0">Tackingout</h6>
        <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
          <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
            <li class="breadcrumb-item text-white"><i class="fas fa-folder-open"></i></li>
            <li class="breadcrumb-item"><a href="{{ route('tackingout.index') }}">Tackingout</a></li>
...
<div class="row justify-content-center">
    <div class="col-12 col-lg-8  card ">
        <div class="card-header">
            <h4 class="card-title">Edit Tackingout</h4>
        </div>
        <form action="{{ route('tackingout.update', $tackingout->id) }}" method="POST">
            @method('PUT')
            @csrf
        <div class="card-body p-4 row justify-content-center bg-secondary">
            <div class="col-12">
                <div class="form-group @error('no_tacking') has-error has-feedback @enderror">
                    ...
                    <input name="no_tacking" value="{{ old('no_tacking') ?? $tackingout->no_tacking  }}" type="text" class="form-control form-control-alternative" placeholder="No Pengiriman" required>
                    @error('no_tacking') 
                ...
                <div class="form-group @error('tgl_tacking') has-error has-feedback @enderror">
                    ...
                    <input name="tgl_tacking" value="{{ old('tgl_tacking') ??  $tackingout->tgl_tacking }}" type="date" class="form-control form-control-alternative" placeholder="Tgl Pengiriman" required>
                    @error('tgl_tacking') 
                    <small class="form-text text-danger">
                        <strong>{{ $message }}</strong>
                    </small> 
                ...
                <div class="form-group @error('receiveby') has-error has-feedback @enderror">
                    ...
                    <input name="receiveby" value="{{ old('receiveby') ??  $tackingout->receiveby }}" type="text" class="form-control form-control-alternative" placeholder="Penerima" required>
                    @error('receiveby') 
                    <small class="form-text text-danger">
                        <strong>{{ $message }}</strong>
                    </small> 
                ...
                <div class="form-group @error('lokasi') has-error has-feedback @enderror">
                    <label>Lokasi</label>
                    <input name="lokasi" value="{{ old('lokasi') ??  $tackingout->lokasi }}" type="text" class="form-control form-control-alternative" placeholder="Lokasi" required>
                    @error('lokasi') 
                    <small class="form-text text-danger">
                        <strong>{{ $message }}</strong>
                    </small> 
                ...
                <div class="form-group @error('tujuan') has-error has-feedback @enderror">
                    <label>Tujuan</label>
                    <textarea class="form-control form-control-alternative" name="tujuan">{{ old('tujuan') ??  $tackingout->lokasi }}</textarea>
                    @error('tujuan') 
                    <small class="form-text text-danger">
                        <strong>{{ $message }}</strong>
                    </small> 
            ...
            <div class="col-12 row pt-4" id="barang_field">
                ...
                @foreach ($tackingout->details as $key=>$detail)
                <div class="form-group barang col-8">
                    <label>Nama Barang</label>
                    <select name="barang_id[]" class="form-control select2 barang_id">
                        @foreach ($goods as $good)
                        <option value="{{ $good->id }}" {{ $detail->barang_id==$good->id ? 'selected' : ''}}>{{ $good->kd_brg }} - {{ $good->nm_brg }} ({{ $good->qty_sisa}} {{$good->unit}})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group qty col-3">
                    <label>Qty</label>
                    <input name="qty_brg[]"  min="0" type="number" class="form-control form-control-alternative" placeholder="Qty" value="{{ $detail->qty_brg }}" required>
                </div>
